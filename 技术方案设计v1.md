...
### 3.1 Agent 类 (核心大脑)
实现递归的 Think-Act 循环和子代理栈式调用。

```python
class Agent:
    def __init__(self, session: Session, parent: Optional['Agent'] = None):
        ...
        while True:
            ...
            for tool_call in response.tool_calls:
                if tool_call.name == "Task":
                    # === 关键：栈式递归创建子代理 ===
                    sub_agent = Agent(session=Session.create_child(), parent=self)
                    result = await sub_agent.run(tool_call.args['goal'])
                else:
                    # 工具执行 (全异步)
...
class Session:
    ...
    def add_message(self, msg):
        ...
        if self._count_tokens() > self.max_tokens * 0.9:
            self._compress_context()
            
    def _compress_context(self):
        # 策略：保留 System + 最近 10 轮
        # 将中间部分发送给 LLM 请求摘要
        pass
```
...
### 4.2 宿主机 Shell 执行
为了安全且方便，不使用 Docker，而是使用 `subprocess`。

```python
# tools/shell_tools.py
def run_bash(command: str):
    # 1. 黑名单检查
    if is_dangerous(command): # e.g., rm -rf /
